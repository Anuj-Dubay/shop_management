...
import calendar
from database import is_order_window_open, next_window_time
...
def show_restock(shop, today):
    ...
    if not is_order_window_open():
        st.error(f"🕐 Order window is closed. Next window opens at {next_window_time()}")
        st.info("Order windows: 10:00 AM - 6:30 PM  |  12:00 AM - 5:00 AM")
        return
